...
var color = d3.scale.threshold()
    .domain([2.718, 2.896, 3.06, 3.270, 5.0])
    .range(d3.range(5).map(function(d) { return "q" + d + "-5"; }));

// Draw calendar.
var svg = d3.select("#rank-cal").selectAll("svg")
    .data([2012])
  .enter().append("svg")
    .attr("width", width)
    .attr("height", height)
    .attr("class", "rank-colors")
  .append("g")
...
var driving_rank_rect = rect.filter(function(d) { return d in calendar_driving_ranks; })
...
driving_rank_rect.attr("class", function(d) { 
    return "day " + color(calendar_driving_ranks[d]); 
    })
  .select("title")
    .text(function(d) { return d + ": " + calendar_driving_ranks[d]; 
  });


// Generate tooltip for day rectangles.
driving_rank_rect.on("mouseover", function (d){
    var title_text = "<p>" + d + "</p>";
    var pos = $(this).offset();
